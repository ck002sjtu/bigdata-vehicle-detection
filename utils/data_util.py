...
def read_data(filepath, nb_classes, target_image_size=(416, 416)):
	...
	para:
		filepath: ex:/images/imagelist.txt should have the form
			image_path, x, y, w, h, class, x, y, w, h, class
			image_path, x, y, w, h, class
			...
			ex:
			images/image1, 120, 30, 50, 20, 9, 130, 50, 20, 90, 19
			...
	"""
	f = open(filepath)
	line = f.readline()
	...
	while line:
		...
		processed_array, object_mask, object_value = convert_ground_truth(array, image_size=image_size)
		array_all = np.ones((50,),dtype='float32')*(-1)
		array_all[0:len(array)] = processed_array[:]
		y1.append(array_all)
	...
	while True:
		...
		while line:
			...
			for num in data[1:]:
				array.append(float(num))
			array = np.asarray(array)
			processed_array, object_mask, object_value = convert_ground_truth(array, 
...
def convert_ground_truth(boxes, image_size):
	"""
	...
	return:
		...
		object_value: shape (13, 13, 5, 5 ). values like x,y,w,h and class for 
			bounding box is saved in this variable. x, y corresponds to bx, by,
	...
	for i in xrange(len(boxes)/5):
		...
		array.extend([x,y,w,h,box[4]])
		if maximum_iou>0:
			index1 = np.int((box[0]+box[2]/2)/(width/13.))
			index2 = np.int((box[1]+box[3]/2)/(height/13.))
			index3 = np.argmax(iou)
			object_mask[index1, index2, index3, 0] = 1
			#object_mask.append([np.floor(box[0]/32.), np.floor(box[1]/32.), index])
			object_value[index1, index2, index3] = [x,y,w,h,box[4]]
			#object_value.append([np.floor(box[0]/32.), np.floor(box[1]/32.), index, x,y,w,h,box[4]])

	return array, object_mask, object_value